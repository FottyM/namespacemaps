  ...
  switch (action.type) {
    case 'LOAD_TASKS':
      ...
      return {
        ...state,
        tasks: newtasks,
        lastUpdatedTaskTime: action.payload.lastUpdatedTaskTime
      };
...
const filtertask = t =>
  (t.state === 'assigned' && t.address.location.coordinates.length > 0) ||
  (t.state === 'unassigned' && t.address.location.coordinates.length > 0);
